...
terraform {
  backend "s3" {
    bucket = "us-xxxxx-xxxx-xxx"
    key    = "sbox/buildbot/sbox.tfstate"
    region = "us-west-2"
    profile = "dev1"
  }
}
...
module "sandbox_sg_ssh_access" {
    ...
    sgrules_depends_on = [module.buildbot_vpc.sandbox_vpc_id]

    sandbox_name = var.sandbox_name
    aws_vpc_cidr_block = var.cidr_block
    sandbox_vpc_id = module.buildbot_vpc.sandbox_vpc_id
...
module "sandbox_asg1" {
    ...
    asg_depends_on = [module.buildbot_vpc.sandbox_vpc_id, 
    ...
    subnet_az1_id = module.buildbot_vpc.sandbox_subnet_az1_id
    subnet_az2_id = module.buildbot_vpc.sandbox_subnet_az2_id
